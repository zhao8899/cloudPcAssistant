...
// 移动bar的样式
const tabBarStyle = computed<Record<string, string | number>>(() => {
...
const init = async () => {
    // 获取tabs组件的尺寸信息
    const tabRect = (await getRect('#' + id.value, false, ctx)) as {
        left: number
        width: number
    }
    // tabs组件距离屏幕左边的宽度
...
const getTabRect = () => {
    ...
    // 执行查询，一次性获取多个结果
    query.exec((res: Array<{ left: number; width: number }>) => {
...
// 子组件调用此函数而产生的事件通信
const handleChange = (event: any, updateRender: (active: boolean) => void) => {
